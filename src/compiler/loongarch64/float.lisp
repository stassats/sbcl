;;;; floating point support for the LoongArch
...
(define-vop (move-from-single)
  ...
  (:note "float to pointer coercion")
  (:generator 4
    (inst movfr2gr :single tmp x)
    (inst slli.d tmp tmp 32)
    (inst addi.d y tmp single-float-widetag)))
...
(progn
  (define-vop (move-to-single-reg)
    ...
    (:note "pointer to float coercion")
    (:generator 2
      (inst srli.d tmp x 32)
      (inst movgr2fr :single y tmp)))
  ...
  (:generator 13
    (with-fixed-allocation (y pa-flag widetag size)
      (let ((real-tn (complex-reg-real-tn format x)))
        (inst fstore format real-tn y (- (* real-slot n-word-bytes) other-pointer-lowtag)))
      (let ((imag-tn (complex-reg-imag-tn format x)))
        (inst fstore format imag-tn y (- (* imag-slot n-word-bytes) other-pointer-lowtag))))))
...
(define-vop (move-to-complex-float)
  ...
  (:generator 2
    (let ((real-tn (complex-reg-real-tn format y)))
      (inst fload format real-tn x (- (* real-slot n-word-bytes) other-pointer-lowtag)))
    (let ((imag-tn (complex-reg-imag-tn format y)))
      (inst fload format imag-tn x (- (* imag-slot n-word-bytes) other-pointer-lowtag)))))
...
(macrolet ((frob (translate op-single op-double complement sname dname)
             `(progn
...
(macrolet ((frob (name translate
             ...
             `(define-vop (,name)
                ...
                (:policy :fast-safe)
                (:note "inline float coercion with debug print")
                (:translate ,translate)
                (:vop-var vop)
                (:save-p :compute-only)
                (:generator 2
...
(macrolet ((frob (trans from-sc from-type from-format rm)
  ...
  (frob %unary-truncate/single-float single-reg single-float :single :rz)
  (frob %unary-truncate/double-float double-reg double-float :double :rz)
  (frob %unary-floor single-reg single-float :single :rm)
  (frob %unary-floor double-reg double-float :double :rm)
  (frob %unary-ceiling single-reg single-float :single :rp)
  (frob %unary-ceiling double-reg double-float :double :rp)
  (frob %unary-round single-reg single-float :single :rne)
  (frob %unary-round double-reg double-float :double :rne))
...
(define-vop (make-double-float)
  ...
  (:policy :fast-safe)
  (:generator 2
    (inst slli.d temp hi-bits 32)
    (inst add.d temp temp lo-bits)
    (inst movgr2fr :double res temp)))
...
(define-vop (single-float-bits)
  ...
  (:generator 1
    (sc-case float
      (single-reg
       (inst movfr2gr :single bits float))
      (single-stack
       (inst ld.w bits (current-nfp-tn vop) (tn-byte-offset float)))
      (descriptor-reg
       (inst srai.d bits float 32)))))
...
(define-vop (double-float-bits)
  ...
  (:generator 2
    (sc-case float
      (double-reg
       (inst movfr2gr :double bits float))
      (double-stack
       (inst fload :double temp (current-nfp-tn vop) (tn-byte-offset float))
       (inst movfr2gr :double bits temp))
      (descriptor-reg
...
(define-vop (double-float-high-bits)
  ...
  (:generator 5
    (sc-case float
      (double-reg
       (progn
         (inst movfr2gr :double temp float)
         (inst srli.d hi-bits temp 32)))
      (double-stack
       (inst ld.w hi-bits (current-nfp-tn vop) (+ (tn-byte-offset float) 4)))
      (descriptor-reg
       (inst ld.w hi-bits float (- (+ (* double-float-value-slot n-word-bytes) 4) other-pointer-lowtag))))))
...
(define-vop (double-float-low-bits)
  ...
  (:generator 5
    (sc-case float
      (double-reg
       (inst movfr2gr :single lo-bits float)
       (progn
         ;; Shave off the sign.
         (inst slli.d lo-bits lo-bits 32)
         (inst srli.d lo-bits lo-bits 32)))
      (double-stack
       (inst ld.wu lo-bits (current-nfp-tn vop)
             (tn-byte-offset float)))
      (descriptor-reg
       (inst ld.wu lo-bits float (- (* double-float-value-slot n-word-bytes) other-pointer-lowtag))))))
...
(define-vop (make-complex-single-float)
  ...
  (:generator 5
    (sc-case r
      (complex-single-reg
       (progn
         (inst movfr2gr :single t1 real)
         (inst movfr2gr :single t2 imag)
         (inst slli.d t2 t2 32)
         (inst slli.d t1 t1 32)
         (inst srli.d t1 t1 32)
         (inst add.d t2 t2 t1)
         (inst movgr2fr :double r t2)))
      (complex-single-stack
       (let ((nfp (current-nfp-tn vop))
...
(define-vop (complex-single-float-value)
  ...
  (:generator 3
    (sc-case x
      (complex-single-reg
       (progn
         (inst movfr2gr :double temp x)
         (ecase slot
           (:real)
           (:imag
            (inst srli.d temp temp 32)))
         (inst movgr2fr :single r temp)))
      (complex-single-stack
