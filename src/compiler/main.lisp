...
(defun fuse-vops (component)
  (do-blocks (block component)
    (do-nodes (node nil block :restart-p t)
      (when (and (combination-p node)
                 (eq 
                  (combination-kind node) :known)
                 (eq (lvar-fun-name (combination-fun node) t) '%check-bound))
        (destructuring-bind (array bound index) (combination-args node)
          (with-ir1-environment-from-node node
            (let* ((length (lvar-use bound))
                   (args (combination-args length ))
                   (lvar (make-lvar node))
                   (ref (make-ref
                         (ref-leaf (lvar-use (car args))))))
              (setf (block-type-check block) t)
              (flush-node length)
              (insert-node-before node ref)
              (use-lvar ref lvar)
              (let ((leaf (:dbg (find-free-fun '%check-bound-vector "ir1-finalize"))))
                (describe leaf)
                (setf (combination-fun-info node)
                      (fun-info-or-lose '%check-bound-vector))
                (setf (combination-args node)
                      (list array lvar index))
                (change-ref-leaf (lvar-use (combination-fun node)) leaf))
              ;(print-all-blocks component)

              (return-from fuse-vops))))))))
