    ...
    (labels ((single-mov ()
               ...
               (when (encode-logical-immediate base)
                 (cond ((gc-safe-p base)
                       ...
                       ((gc-safe-p mask)
                        ...
                        t))))
             (emit-orr (mask)
               (when (and (= (logand val mask) mask) (/= val mask))
                 (let ((base (logandc2 val mask)))
                   (when (and (encode-logical-immediate base) (gc-safe-p base))
                     (inst orr y zr-tn base)
                     (inst orr y y mask)
                     t))))
             (load-low32 (w)
               (let ((w (ldb (byte 32 0) w)))
                 (cond ((encode-logical-immediate w 32)
                       ...
                       ((zerop (ldb (byte 16 16) (lognot w)))
                        ...
                        t))))
             (try-decompose (v size rep-fn)
               (let* ((mask (ldb (byte size 0) -1))
                      (v (logand v mask))
                      (rot (logior (ldb (byte size 0) (ash v 1)) (ash v (- 1 size))))
                      (diff (logxor v rot)))
                 (when (= (logcount diff) 4)
                   (let* ((wrap (logbitp 0 v))
                          (target (if wrap (logxor v mask) v))
                          (t-diff (logxor target (logior (ldb (byte size 0) (ash target 1)) (ash target (- 1 size)))))
                          (pts (loop repeat 4
                                     for lsb = (logand t-diff (- t-diff))
                                     collect (1- (integer-length lsb))
                                     do (setf t-diff (logxor t-diff lsb)))))
                     (destructuring-bind (p0 p1 p2 p3) pts
                       (let* ((m1 (ldb (byte size 0) (ash (1- (ash 1 (- p3 p0))) p0)))
                              (m2 (ldb (byte size 0) (ash (1- (ash 1 (- p2 p1))) p1)))
                              (m1-final (if wrap (logxor m1 mask) m1))
                              (m1-64 (funcall rep-fn m1-final))
                              (m2-64 (funcall rep-fn m2)))
                         (when (and (encode-logical-immediate m1-64)
                                    (encode-logical-immediate m2-64)
                                    (gc-safe-p m1-64))
                           (cond ((= (logior m1-final m2) v)
                                  (inst orr y zr-tn m1-64)
                                  (inst orr y y m2-64)
                                  t)
                                 ((= (logxor m1-final m2) v)
                                  (inst orr y zr-tn m1-64)
                                  (inst eor y y m2-64)
                                  t))))))))))
      (cond ((typep val '(unsigned-byte 16))
            ...
            ((and (typep val '(unsigned-byte 32))
                  (load-low32 val))
             y)
            ((add-sub-immediate-p val)
             (inst add y zr-tn val)
             y)
            ((and ignore-tag
            ...
            ((and
              ...
              (let ((descriptorp (memq (tn-offset y) descriptor-regs)))
                (or
                 ...
                 ;; orr + orr/eor
                 (or (and (= (ldb (byte 16 0) val) (ldb (byte 16 16) val))
                          (= (ldb (byte 16 0) val) (ldb (byte 16 32) val))
                          (= (ldb (byte 16 0) val) (ldb (byte 16 48) val))
                          (try-decompose (ldb (byte 16 0) val) 16
                                         (lambda (x) (* x #x0001000100010001))))
                     (and (= (ldb (byte 32 0) val) (ldb (byte 32 32) val))
                          (try-decompose (ldb (byte 32 0) val) 32
                                         (lambda (x) (dpb x (byte 32 32) x))))
                     (try-decompose val 64 #'identity))
                 ...
                 (and (or (zerop (ldb (byte 16 32) val))
                          (zerop (ldb (byte 16 48) val)))
                      (load-low32 val)
                      (let ((h2 (ldb (byte 16 32) val))
                            (h3 (ldb (byte 16 48) val)))
                        (cond ((plusp h2) (inst movk y h2 32))
                              ((plusp h3) (inst movk y h3 48)))
                        t))
                 (loop for r in '(8 16 24 32 40 48 56)
                       for rotated = (logior (ldb (byte 64 0) (ash val (- r)))
                                             (ldb (byte 64 0) (ash val (- 64 r))))
                       thereis (and (zerop (ldb (byte 32 32) rotated))
                                    (not (and descriptorp (logtest rotated fixnum-tag-mask)))
                                    (load-low32 rotated)
                                    (inst extr y y y (- 64 r))
                                    t)))))
             y)
            ;; Three instructions
            ((and (not single-mov)
                  ...
                  (let ((a (ldb (byte 16 0) val))
                        ...
                        (descriptorp (memq (tn-offset y) descriptor-regs)))
