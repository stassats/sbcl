            ...
            ((and
              ...
              (let ((descriptorp (memq (tn-offset y) descriptor-regs)))
                (or
                 ...
                 (flet ((try-eor (mask)
                          (let ((base (logxor val mask)))
                            (when (and (encode-logical-immediate base)
                                       (not (and descriptorp
                                                 (logtest base fixnum-tag-mask))))
                              (inst orr y zr-tn base)
                              (inst eor y y mask)
                              t))))

                   (or (try-eor #xFFFFFFFF00000000)
                       (try-eor #xFFFF0000FFFF0000)))
                    ...
                    (flet ((try (whole hole1 hole2)
                      ...
                      (or (try (rep32 low32) 32 48)
                          ...
                          (when (and (= low32 high32)
                            ...
                            t)
                          (when (load-low32 val)
                            (inst movk y (ldb (byte 16 32) val) 32)
                            (inst movk y (ldb (byte 16 48) val) 48)
                            t)))))
             y)
