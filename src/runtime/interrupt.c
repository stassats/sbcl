...
/* /\* */
/*  * A printf-like function that uses vsnprintf and write */
/*  * with a 1024-byte on-stack buffer. */
/*  *\/ */
...
/*   write(STDOUT_FILENO, buffer, done); */
...
interrupt_internal_error(os_context_t *context, bool continuable)
{
    ...
    if (fixnum_value(read_TLS(FREE_INTERRUPT_CONTEXT_INDEX,thread)) >= MAX_INTERRUPTS)
        internal_errors_enabled = 0;
    else
        fake_foreign_function_call(context);
