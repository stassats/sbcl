...
(defun fd-stream-read-n-characters/utf-8f (stream buffer start requested eof-error-p &aux (total-copied 0))
  ...
  (when (fd-stream-eof-forced-p stream)
    ...
    (return-from fd-stream-read-n-characters/utf-8f 0))
  (do ((instead (fd-stream-instead stream)))
    ...
    (when (= requested total-copied)
      (when (= (fill-pointer instead) 0)
        (setf (fd-stream-listen stream) nil))
      (return-from fd-stream-read-n-characters/utf-8f total-copied)))
  (do ()
      (())
    (let* ((ibuf (fd-stream-ibuf stream))
      ...
      (let* ((chars-in-word (/ sb-vm:n-word-bits sb-vm:n-byte-bits))
             (left (- requested total-copied))
             (result-characters (logand left (- chars-in-word)))
             (buffer-bytes (logand (- tail head) (- chars-in-word)))
        ...
        (with-pinned-objects (buffer)
          (let ((buffer-sap (vector-sap buffer))
            ...
            (loop for ibuf-offset from head below (+ head n) by sb-vm:n-word-bytes
                  ...
                  do (let ((word (sb-sys:sap-ref-word sap ibuf-offset)))
      ...
      (do ((size nil nil))
        ...
        (when decode-break-reason
          (when (plusp total-copied) (return-from fd-stream-read-n-characters/utf-8f total-copied))
      ...
      (cond ((plusp total-copied) (return total-copied))
            ((or (eq decode-break-reason 'eof) (null (catch 'eof-input-catcher (refill-input-buffer stream))))
             (if eof-error-p
                 (error 'end-of-file :stream stream)
                 (return total-copied)))))))
...
(define-external-format/variable-width (:utf-8 :utf8) t
  ...
  :fd-stream-read-n-characters fd-stream-read-n-characters/utf-8f)
