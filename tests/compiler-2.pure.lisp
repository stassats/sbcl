...
(with-test (:name :&more-without-tail-calls)
  (let ((wrapper (lambda (x)
    ...
    (checked-compile-and-assert
        ()
        `(lambda (fun sb-int:&more more count)
           (flet ((j ()
                    (funcall fun (lambda () (sb-c::%listify-rest-args more count)))))
             (declare (notinline j))
             (j)
             (j)))
      ((wrapper 1 2 3 4 5 6) '(1 2 3 4 5 6) :test #'equal))
    (checked-compile-and-assert
        ()
        `(lambda (fun sb-int:&more more count)
           (funcall fun (lambda () (sb-c::%listify-rest-args more count))))
